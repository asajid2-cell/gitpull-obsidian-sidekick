# Security

## Reporting

Open a private GitHub security advisory if available, or contact the repository owner through GitHub.

Do not publish GitHub tokens, vault contents, or device screenshots in public issues.

## Token Guidance

Use a fine-grained GitHub token with read-only repository contents access. Revoke and replace the token if a device is lost or a token is pasted into the wrong app.
