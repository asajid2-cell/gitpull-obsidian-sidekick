...
The token fallback is still available for builds without browser sign-in. A read-only fine-grained GitHub token is recommended when using that fallback.
