...
- Connected Android tests: PASS, 10 tests, 0 failures on `Pixel_9_Pro_XL(AVD) - 16`
- Lint: PASS, 0 errors
...
- GitHub sign-in UX: PASS, pending sign-in keeps the code visible across activity recreation, supports copying it, keeps `Check sign-in` available, polls again when the app resumes, completes approved sign-in in the UI, saves auth state, and loads a repository list.
