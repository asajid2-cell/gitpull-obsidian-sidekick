...
- Connected Android tests: PASS, 12 tests, 0 failures on `Pixel_9_Pro_XL(AVD) - 16`
- Lint: PASS, 0 errors
...
- Live GitHub OAuth Device Flow smoke: PASS, `POST https://github.com/login/device/code` returned a user code and `https://github.com/login/device`; a user-approved code returned a bearer access token and loaded `GET https://api.github.com/user/repos`.
- GitHub sign-in UX: PASS, pending sign-in is committed synchronously, keeps the code visible across activity recreation, supports copying it, keeps `Check sign-in` available, keeps the code active after transient poll failures, polls again when the app resumes, completes approved sign-in in the UI, saves auth state, and loads a repository list.
